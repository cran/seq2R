subroutine fonseca(n,hA,hT,p,A,T,X,W,Xb,AT,ATi,ATs,AT1,AT1i,AT1s,&
critical,criticali,criticals,nboot,kbin,kernel,nh)

!!DEC$ ATTRIBUTES DLLEXPORT:: fonseca
!!DEC$ ATTRIBUTES C, REFERENCE, ALIAS:'fonseca_' :: fonseca

implicit none
integer*4 iend,i,ifold,p,iboot,j,icont,imin,imax,Xmin(200),&
Xmax(200),nh,kernel,n,kbin,nboot,seed
character*30 fichero
character*50 salida

double precision, allocatable:: Fold(:)
double precision A(n),C(n),T(n),Y(n),G(n),X(n),W(n),hA,hT,Vaux(20000),Signi(5000),&
critical(kbin),vmax,criticalboot(kbin,nboot), &
criticali(kbin),criticals(kbin),Ati(kbin),Ats(kbin),At1i(kbin),&
At1s(kbin),xb(kbin),At(kbin),At1(kbin),marta(100)
...
!! Pb(:,3) para estimation, first and second der.
...
!WRITE(*, '(a, g20.12)') ' hA = ', hA
hT=hA
call RFAST_H(X,T,W,n,hT,p,Xb,PbT,kbin,kernel,nh)
!WRITE(*, '(a, g20.12)') ' hT = ', hT
!call RFAST(X,A,n,W,hA,p,Xb,PbA,kbin)
!call RFAST(X,T,n,W,hT,p,Xb,PbT,kbin)
...
call Interpola (Xb,PbA(1,1),kbin,X,PredA,Vaux,n) ! vaux puede que sea primera derivada
call Interpola (Xb,PbT(1,1),kbin,X,PredT,Vaux,n)


!CALL SEED(1)

u=rand()
if(u.lt.0.2) seed=100
if(u.ge.0.2.and.u.lt.0.3) seed=200
if(u.ge.0.3.and.u.lt.0.4) seed=300
if(u.ge.0.4.and.u.lt.0.5) seed=400
if(u.ge.0.5.and.u.lt.0.6) seed=500
if(u.ge.0.6.and.u.lt.0.7) seed=600
if(u.ge.0.7.and.u.lt.0.8) seed=700
if(u.ge.0.8.and.u.lt.0.9) seed=800
if(u.ge.0.9) seed=900


call srand(seed)
do iboot=1,nboot
!    write (*,*) iboot ! no puedo pintar por pantalla cuando hago un paquete, se pone tonto
	do i=1,n
		Aboot(i)=Bernoulli(predA(i))
		Tboot(i)=Bernoulli(predT(i))
		if(iboot.eq.1) then
do j=1,100
marta(j)=Aboot(j)
end do
	...
	!call RFAST(X,Aboot,n,W,hA,p,Xb,PbA,kbin)
	!call RFAST(X,Tboot,n,W,hT,p,Xb,PbT,kbin)
...
! Intervalos de confianza para AT
do i=1,kbin
	do j=1,nboot
		Vaux(j)=Atboot(i,j)
	end do
    call IC(At(i),Vaux,nboot,ATi(i),Ats(i))
end do


! Intervalos de confianza  AT^1
call Interpola (Xb,At,kbin,Xb,Vaux,At1,kbin)
do iboot=1,nboot
    do i=1,kbin
		Vaux(i)=Atboot(i,iboot)
	end do
	call Interpola (Xb,Vaux,kbin,Xb,Vaux,At1boot(1,iboot),kbin)
end do
...
do i=1,kbin
    ...
    call IC(At1(i),Vaux,nboot,AT1i(i),At1s(i))
end do




! No meter en el paquete
!write (salida,'(a6,a30)') 'plots.txt'
!write (*,*) salida
!open (1,file=salida)
!write (1,'(10(a18,1x))') 't','m','li','ls','m1','li1','ls1'
!do i=1,kbin
!	write (1,'(10(f18.8,1x))') Xb(i),At(i),Ati(i),Ats(i),At1(i),At1i(i),At1s(i)
!end do 
!close(1)
...
do i=1,icont
	do j=1,nboot
		Vaux(j)=criticalboot(i,j)
	end do
    call IC(critical(i),Vaux,nboot,criticali(i),criticals(i))
...
!write (salida,'(a9,a30)') 'critical_',fichero
!write (*,*) salida
!open (1,file=salida)
!write (1,'(10(a12,1x))') 't','li','ls'
!do i=1,icont
!	write (1,'(10(f12.4,1x))') critical(i),criticali(i),criticals(i)
!end do
!close(1)
...
double precision function Bernoulli(p)
implicit none
integer seed
double precision p    !,seed !,rand
REAL*4 u,rand
...
subroutine prepara(fichero)
implicit none
integer*4 i,iend,i1,i2
character aux(20000)
character Y1(10000),Y2(10000)
character*30 fichero
open (1,file=fichero)
read (1,'(20000a)')  aux
close(1)
...
!write (1,'(3(a2,1x))') 'Y1','Y2' 
!do i=1,i1
!	write (1,'(3(a2,1x))') Y1(i),Y2(i) 
!end do
!close(1)
...
INTEGER*4, SAVE                :: nobs, ncol, r_dim
INTEGER*4, ALLOCATABLE, SAVE   :: vorder(:), row_ptr(:)
...
INTEGER*4, PARAMETER           :: dp = SELECTED_REAL_KIND(12,60)
...
INTEGER*4, INTENT(IN)                   :: nreq, dimcov
INTEGER*4, INTENT(OUT)                  :: ifault
...
INTEGER*4                :: dim_rinv, pos, row, start, pos2, col, pos1, k
double precision              :: total
...
INTEGER*4    :: pos, row, col, start, k, pos1, pos2
double precision  :: total
...
INTEGER*4, INTENT(IN)                  :: in, dimc
INTEGER*4, INTENT(OUT)                 :: ifault
double precision, DIMENSION(:), INTENT(OUT) :: cormat, ycorr

!     Local variables.

INTEGER*4    :: base_pos, pos, row, col, col1, col2, pos1, pos2
...
INTEGER*4, INTENT(IN)    :: from, to
INTEGER*4, INTENT(OUT)   :: ifault

!     Local variables

double precision  :: d1, d2, x, d1new, d2new, cbar, sbar, y
INTEGER*4    :: m, first, last, inc, m1, m2, mp1, col, pos, row
...
INTEGER*4, INTENT(IN)               :: n, pos1
INTEGER*4, DIMENSION(:), INTENT(IN) :: list
INTEGER*4, INTENT(OUT)              :: ifault

!     Local variables.

INTEGER*4    :: next, i, l, j
...
USE lsq
IMPLICIT NONE
INTEGER*4             :: i, ier, iostatus, j, m, n,nvar,iopt
double precision          :: x(n,nvar), y(n),W(n), xrow(0:nvar+1), wt = 1.0_dp, beta(0:nvar+1),var, &
...
!subroutine Sample(X,Y,W,X2,Y2,W2,n)
!implicit none
!integer n,i,ii
!double precision u,u2
!double precision X(n),Y(n),W(n),X2(n),Y2(n),W2(n)
!real,external::rand
...
implicit none
integer*4 n,i,j,kbin,p,ifcv,icont,ih,ih0,i0,nh,kernel
...
implicit none
integer*4 n,i,j,kbin
double precision x(n),y(n),W(n),Xb(kbin),Yb(kbin),Wb(kbin),&
d1,Area(2),dis1,dis2


!CONSTRUCCION DE LA MUESTRA BINNING
	Wb=0
	Yb=0
	do i=1,n	
		if (W(i).gt.0) then
...
!	VENTANA1D
!**********************************************************
!* h: ventana, selecciOn por cv (h=-1). Valores de 0 a 1.
!* kernel: tipo de nucleo (1=epanech, 2=triang, 3=gaussian)
!* nh: grid de busqueda de ventanas, de 0 a 1.
!**********************************************************
subroutine Ventana1D(X,Y,W,n,h,p,hmin,hmax,nh,rango,kernel)
implicit none
integer*4 i,j,k,icont,p,iopt,ier,n,ih,istep,nh,ih2,i2,Err(nh),kernel
double precision x(n),x2(n),Y(n),h,tanh,waux,Beta(10),Sterr(20),Ventana(n),hmin,hmax,&
se(20),r2,pred(8),B(3),W(n),x0,hgrid(nh),ErrH(5000),sumW,rango,Erraux(nh),&
WW(n),Ventana2(n),medaux,VT,sumy,sumy2,maxr2,minr2,angulo
double precision,allocatable::ErrCV(:,:),Predh(:,:),WCV(:)
integer*4,external::which_min
allocate(ErrCV(n,nh),Predh(n,nh),WCV(n))


! ESTABLECENSE o GRID de VENTANAS de BUSQUEDA
do ih=1,nh
	hgrid(ih)=hmin+(ih-1)*(hmax-hmin)/(nh-1)
end do


! Para cada punto fanse as estimacions por CV para cada unha das ventanas
!icont=1000
Err=0
...
! Calculanse os erros (globais) obtidos para cada ventana. Para elo calculase
! o erro cadratico medio ponderado polos pesos W iniciais
...
! A continuacion escribese un resume dos erros obtidos. O ficheiro ten 3 columnas:
! Ventana; error absoluto: % de incremento de erro en relacion a ventana optima
! (loxicamente, na ultima columna a ventana optima tera un valor de 0) 
...
!do i=nh,ih,-1 !***
!do i=ih,nh
!	if (Errh(i)+0.001.ge.Errh(ih)) then   !Javier
!			h=hgrid(i)
			!	goto 33
!			end if
!end do 
...
!do i=nh,1,-1
do i=ih,nh
	if (Errh(i)+((maxr2-minr2)*0.1).ge.Errh(ih)) then  ! hacemos rango y luego el 10% de ese valor
		h=hgrid(i)
!		goto 33
	end if
end do
...
!if (0.le.h2.and.h2.le.1) then  !permite elegir ventanas [0,1]
if (h2.gt.0) then !permite elegir ventanas mayores que uno
3		continue	
		icont=0	
		do i=1,n
			u=((X(i)-x0)/rango) /h2 
...
subroutine Rfast_(X,Y,n,W,h,p,Xb,Yb,Wb,Pb,kbin,ifcv)
implicit none
integer*4 n,i,j,kbin,p,ifcv,icont,l
double precision x(n),y(n),W(n),Xb(kbin),Yb(kbin),Wb(kbin),Pred(n),kernel2(0:kbin),&
d1,Area(2),dis1,dis2,X2(kbin),W2(kbin),Pb(kbin,3),m,&
kernel(0:kbin),aux,h,sumw,tanh,B(10),V(kbin),h2
real*4 :: pi = 3.1415927 
double precision,external::csval,fregl
...
!PESOS NUCLEO
	if (h2.ge.0.and.h2.le.100) then
		kernel=0
		dis1=(Xb(2)-Xb(1))/(xb(kbin)-xb(1))
		!tanh=dsin(h2*.9)/dcos(h2*.9)
		!tanh=sind(h2*.9)/cosd(h2*.9)
		tanh=sin(h2*.9*(pi/180))/cos(h2*.9*(pi/180))
		do i=0,kbin
			kernel(i)=1-tanh*i*dis1
			if (kernel(i).lt.0) kernel(i)=0
		end do	
	else
		!write (*,*) 'error en ventana'
		read (*,*) 
		!stop
	end if

	if (ifcv.gt.0) kernel(0)=0 
...
1		continue
		...
		do j=1,kbin
			if (Wb(j)*Kernel2(abs(i-j)).gt.0) then
				icont=icont+1
				X2(icont)=Xb(j)-Xb(i)
				W2(icont)=Wb(j)*Kernel2(abs(i-j))
				sumw=sumw+W2(icont)
				V(icont)=yb(j)
			end if
		end do
		W2=W2/sumw
			...
			call Reglineal (X2,V,W2,icont,p,B)
			pb(i,1)=B(1)
			if (p.ge.2) then
				Pb(i,2)=B(2)
				pb(i,3)=2*B(3)
...
subroutine ECM(Y,Pred,W,n,error)
implicit none
integer*4 n,i
double precision Y(n),pred(n),W(n),error,sumw
error=0
sumw=0
do i=1,n
	sumw=sumw+W(i)
	error=error+W(i)*(Pred(i)-Y(i))**2
end do
error=error/sumw
end
